...
//Reimplemented mouseReleaseEvent so you can draw vectors by pressing and releasing mouse left button
void CoordPlaneScene::mouseReleaseEvent(QGraphicsSceneMouseEvent * event)
{
    QGraphicsScene::mouseReleaseEvent(event);
    if (QLineF(event->scenePos(), event->buttonDownScenePos(Qt::LeftButton)).length() < QApplication::startDragDistance())
        return;
    if(itemAt(event->scenePos(), QTransform()) == nullptr && event->button() == Qt::LeftButton)
        addItem(new GraphicsVector(event->buttonDownScenePos(Qt::LeftButton).x(), event->buttonDownScenePos(Qt::LeftButton).y(), event->scenePos().x(), event->scenePos().y()));
}
